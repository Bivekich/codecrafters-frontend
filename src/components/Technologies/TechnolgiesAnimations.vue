...
onMounted((): void => {
  ...
  const subcontents: NodeListOf<Element> = component.querySelectorAll('.subcontent')

  const originPositions: string[] = []
  const clientScreen: number = window.screen.height
  let animationAvailable: boolean = true

  subcontents.forEach((subcontent: Element, i: number): void => {
    if ((i + 1) % 2 === 0) {
  ...
  const gradients: NodeListOf<Element> = component.querySelectorAll('.gradient')

  gradients.forEach((gradient: Element): void => {
